...
@torch.no_grad()
def verify(model_ori, model_v1, model_v2, args, data_ub=None, data_lb=None, spec=None):
    device = args.device
...
@torch.no_grad()
def nn4sys_verification(model_ori, vnnlib, args, onnx_path):
    ...
    for k, spec in enumerate(vnnlib):
        ...
        res = verify(model_ori, model_v1, model_v2, args, 
            data_ub=data_max, data_lb=data_min, spec=spec)
        if len(res) == 3:
            assert res[2] == 'unsafe'
            unsafe = True
            break
    ...
    if unsafe:
        res = 'unsafe'
    elif all_verified and time.time() - start_time <= args.timeout - 5:
        res = 'verified'
    else:
        res = 'timeout'
